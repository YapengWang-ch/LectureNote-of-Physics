...
\section{Efrenfest Theorem}
量子系统随时间的演化服从薛定谔方程，进而可以推导出物理量$F$对应的演化规律：
\begin{graybox}
    某量子系统哈密顿量$\op{H}$不显含时间, 力学量$F$对应的算符为$\op{F}$.对于量子态$\ket{\psi,t}$,力学量$\op{F}$的均值为：
    \begin{equation}
        \left<F\right>=\bra{\psi,t}\op{F}\ket{\psi,t}.
    \end{equation}
    那么再薛定谔绘景中，其均值对时间的微分为：
    \begin{equation}
        \begin{aligned}
            \dd{}{t}\left[\bra{\psi,t}\op{F}\ket{\psi,t}\right]&=\dd{\bra{\psi,t}}{t}\op{F}\ket{\psi,t}+\bra{\psi,t}\op{F}\dd{\ket{\psi,t}}{t}\\
            &=\bra{\psi,t}\left[\frac{i\op{H}}{\hbar}\op{F}-\op{F}\frac{i\op{H}}{\hbar}\right]\ket{\psi,t}\\
            &=\frac{1}{i\hbar}\bra{\psi,t}\com{\op{F}}{\op{H}}\ket{\psi,t}.
        \end{aligned}
    \end{equation}

    即
    \begin{equation}
        \dd{\expectket{F}}{t}=\frac{1}{i\hbar}\expectket{\com{\op{F}}{\op{H}}}.
    \end{equation}
\end{graybox}

对于自由粒子的哈密顿量$\op{H}=\frac{\op{P}^2}{2m}$，计算各个算符与哈密顿量的对易可以得到:
\begin{align}
    \com{\op{P}}{\op{H}}&=0.\\
    \com{\op{x}}{\op{H}}&=\frac{i\hbar}{m}\op{P}.\\
    \com{\op{x}^2}{\op{H}}&=\frac{i\hbar}{m}\anticom{\op{x}}{\op{P}}.
\end{align}
因此可以得到：
\begin{equation}
    \dd{\expect{x}}{t}=\frac{\expect{p}}{m}
\end{equation}
\begin{equation}
    \ddto{}{t}\left(\expect{x^2}\right)=\frac{2\expect{p^2}}{m^2}.
\end{equation}
...
代入径向方程中：
\begin{equation}
    u''(r)+\left[\frac{2m}{\hbar^2}\left(E+\frac{e^2}{4\pi\varepsilon_0 r}\right)-\frac{l(l+1)}{r^2}\right]u(r)=0.
\end{equation}
引入无量纲量$\rho=\sqrt{-\frac{8mE}{\hbar^2}}r$, 可以将方程简化为：
\begin{equation}
    u''(\rho)+\left[-\frac{1}{4}+\frac{\eta}{\rho}-\frac{l(l+1)}{\rho^2}\right]u(\rho)=0, \quad \eta=\frac{me^2}{4\pi\varepsilon_0\hbar^2}\sqrt{-\frac{\hbar^2}{8mE}}.
\end{equation}

这个方程的解为\gls{Laguerre_Polynomial}(详见附录\ref{Laguerre_Polynomial}),其存在收敛解的条件为：

\begin{equation}
    \eta=n_r+l+1, \quad n_r=0,1,2,3,\cdots.
\end{equation}

因此可以得到氢原子能量的本征值为：
\begin{equation}
    E_n=-\frac{me^4}{32\pi^2\varepsilon_0^2\hbar^2}\frac{1}{n^2}, \quad n=n_r+l+1=1,2,3,\cdots.
\end{equation}

氢原子的基态能量为$E_1=-\frac{me^4}{32\pi^2\varepsilon_0^2\hbar^2}$, 其对应的波函数为：
\begin{equation}
    \psi_{100}(r,\theta,\phi)=\frac{1}{\sqrt{\pi a_0^3}}e^{-r/a_0}, \quad a_0=\frac{4\pi\varepsilon_0\hbar^2}{me^2}.
\end{equation}
其中$a_0$称为玻尔半径，氢原子能量的本征值可以写成：
\begin{equation}
    E_n=-\frac{\hbar^2}{2ma_0^2}\frac{1}{n^2}, \quad n=1,2,3,\cdots.
\end{equation}

第一激发态的能量为$E_2=-\frac{me^4}{128\pi^2\varepsilon_0^2\hbar^2}$, 其对应的波函数为：
\begin{equation}
